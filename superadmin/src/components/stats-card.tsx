...
export const StatsCard = ({ name, value, icon: Icon, color, index, onClick }: StatsCardProps) => (
  <motion.div
    ...
    onClick={onClick}
    className={`p-6 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm ${onClick ? 'cursor-pointer hover:border-primary/80/50 hover:shadow-lg transition-all active:scale-[0.98] group' : ''}`}
  >
